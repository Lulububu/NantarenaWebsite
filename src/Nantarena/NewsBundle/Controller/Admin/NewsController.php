    ...
    public function editAction(Request $request, News $news)
    {
        ...
        if ($form->isValid()) {
            try {
                ...
                $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('news.admin.news.edit.flash_success', array(
                    '%title%' => $news->getTitle(),
                )));

                return $this->redirect($this->get('nantarena_news.news_manager')->getEditPath($news));
            } catch (\Exception $e) {
                $this->get('session')->getFlashBag()->add('error', $this->get('translator')->trans('news.admin.news.edit.flash_error', array(
                    '%title%' => $news->getTitle(),
                )));
            }
        }
    ...
    public function deleteAction(Request $request, News $news)
    {
        ...
        if ($form->isValid()) {
            if ($form->get('id')->getData() == $news->getId()) {
                ...
                $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('news.admin.news.delete.flash_success'));
            } else {
                $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('news.admin.news.delete.flash_error'));
            }
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            try {
                ...
                $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('news.admin.news.create.flash_success', array(
                    '%title%' => $news->getTitle(),
                )));

                return $this->redirect($this->generateUrl('nantarena_news_admin_index'));
            } catch (ORMException $e) {
                $this->get('session')->getFlashBag()->add('error', $this->get('translator')->trans('news.admin.news.create.flash_error', array(
                    '%title%' => $news->getTitle(),
                )));
            }
        }
